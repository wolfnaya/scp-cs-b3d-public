...
Function UpdateQuickLoading%()
	If QuickLoadPercent > -1
		If QuickLoadPercent > 99
			...
			EndIf
		EndIf
		If szl\Loading = 0 Then QuickLoadEvents()
	Else
		QuickLoadPercent = -1
...
Function RenderQuickLoading%()
	...
	If QuickLoadPercent > -1 And (opt\HUDEnabled And me\ShowHUD)
		MidHandle(t\IconID[Icon_Quick_Loading])
		DrawImage(t\IconID[Icon_Quick_Loading], opt\GraphicWidth - CoordEx, opt\GraphicHeight - (150 * MenuScale))
		Color(255, 255, 255)
...
Function UpdateEvents%()
	...
	Local p.Particles, n.NPCs, r.Rooms, e.Events, e2.Events, de.Decals, w.WayPoints, pr.Props, l.Lights, se.SoundEmitters
		...
		Select(e\EventID)
			Case e_cont1_173
				...
				If e\EventState = 0.0
					...
					PlayAnnouncement("SFX\Intercom\Scripted\Announc_Breach_Protocol.ogg")
					...
					If PlayerRoom = e\room
						...
						If e\room\NPC[0] = Null
							...
							e\room\NPC[3]\State = 8.0 : e\room\NPC[3]\HP = 0
							...
							e\room\NPC[4]\State3 = -1.0 : e\room\NPC[4]\HP = 0
							...
							e\room\NPC[5]\State3 = -1.0 : e\room\NPC[5]\HP = 0
				...
				Else
						...
						If e\EventState2 = 0.0
							If e\EventState > 900.0 And e\room\RoomDoors[3]\Open
								...
								Else
									...
									If e\room\NPC[2]\State <> 1.0 And me\Health > 0
					...
					If ((e\EventState Mod 600.0 > 300.0) And ((e\EventState + fps\Factor[0]) Mod 600.0 < 300.0))
						...
						If i > 26 
							...
							HideEntity(e\room\RoomAlarmRotors[0]\Light) : HideEntity(e\room\RoomAlarmRotors[0]\Light2)
							HideEntity(e\room\RoomAlarmRotors[0]\LightCone) : HideEntity(e\room\RoomAlarmRotors[0]\LightCone2)
			...
			Case e_cont1_173_intro ; ~ TODO: Write a new code
				...
				If PlayerRoom = e\room
					...
					CanSave = 1
					If me\Health > 0 And e\EventState2 = 0.0
						If e\EventState = 0.0
							For i = 0 To 1
						...
						If e\EventState3 > 0.0
							...
							If e\EventState3 < 170.0
								If e\EventState3 = 1.0
									...
									MusicCHN = StreamSound_Strict("SFX\Music\" + Music[MUS_INTRODUCTION] + ".ogg", opt\CurrMusicVolume * opt\MasterVolume)
					...
					Else
						If e\room\Dist < 8.0
							If I_005\ChanceToSpawn = 3
								...
								e\room\NPC[0]\State = 8.0 : e\room\NPC[0]\HP = 0
...
;							EntityType(it\Collider, HIT_ITEM)
					...
					PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[2]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[3]))
					ToMTElevatorFloor = LowerFloor
					e\EventState4 = UpdateMTElevators(e\EventState4, e\room\RoomDoors[1], e\room\RoomDoors[2], e\room\Objects[2], e\room\Objects[3], e)
				EndIf
			...
			Case e_cont1_106
				...
				If PlayerRoom = e\room
						...
						If EntityY(me\Collider) < (-6900.0) * RoomScale
							...
							If e\EventState = 0.0
								...
								If SoundTransmission
									...
									If d_I\ClosestButton = e\room\Objects[0] And HitKeyUse
										If ChannelPlaying(e\SoundCHN) Then StopChannel(e\SoundCHN) : e\SoundCHN = 0
					...
					Else
						...
						e\room\NPC[0]\State3 = -1.0 : e\room\NPC[0]\HP = 0 : e\room\NPC[0]\HideFromNVG = True
						RotateEntity(e\room\NPC[0]\Collider, 0.0, e\room\Angle + 180.0, 0.0, True)
					...
					PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[2]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[3]))
					ToMTElevatorFloor = LowerFloor
					e\EventState4 = UpdateMTElevators(e\EventState4, e\room\RoomDoors[0], e\room\RoomDoors[1], e\room\Objects[2], e\room\Objects[3], e)
				Else
				...
				If PlayerRoom = e\room
					If e\EventState = 0.0 Lor e\EventStr <> "LoadDone"
						...
						If e\room\RoomDoors[1]\Open Lor e\room\RoomDoors[2]\Open
							...
							e\room\NPC[0]\State3 = -1.0 : e\room\NPC[0]\HP = 0
						...
						Select(e\EventState)
							...
							Case 67.0
								...
								If DistanceSquared(EntityX(me\Collider), EntityX(e\room\Objects[1], True), EntityZ(me\Collider), EntityZ(e\room\Objects[1], True)) < 7.0 And (Not (chs\NoTarget Lor I_268\InvisibilityOn))
									If Rand(100) = 1
										msg\DeathMsg = Format(GetLocalString("death", "205"), Occupation)
										
										DamagePlayer(Rand(12, 16), 0.0, 300.0)
...
;									EntityType(it\Collider, HIT_ITEM)
					...
					If wi\NightVision > 0 Lor wi\SCRAMBLE > 0
						...
						For i = 0 To MaxItemAmount - 1
							If Inventory(i) <> Null
								If (wi\NightVision > 0 And (Inventory(i)\ItemTemplate\TempName = "nvg" Lor Inventory(i)\ItemTemplate\TempName = "nvg3" Lor Inventory(i)\ItemTemplate\TempName = "nvg2")) Lor ((wi\SCRAMBLE = 1 And Inventory(i)\ItemTemplate\TempName = "scramble") Lor (wi\SCRAMBLE = 2 And Inventory(i)\ItemTemplate\TempName = "scramble2"))
									If Inventory(i)\State > 0.0 Lor (wi\NightVision = 3 And Inventory(i)\ItemTemplate\TempName = "nvg2")
						...
						If CoffinDistance < 4.0 And HasBatteryFor895 And I_714\Using <> 2
							...
							ElseIf me\Sanity < -800.0
								If Rand(3) = 1 Then EntityTexture(t\OverlayID[Orl_NVG], t\OverlayTextureID[Orl_Txt_NVG])
								If Rand(6) < 5
									EntityTexture(t\OverlayID[Orl_NVG], mon_I\MonitorOverlayID[Rand(MONITOR_895_OVERLAY_1, MONITOR_895_OVERLAY_6)])
									For i = 0 To MaxItemAmount - 1
										If Inventory(i) <> Null
											If (wi\NightVision > 0 And (Inventory(i)\ItemTemplate\TempName = "nvg" Lor Inventory(i)\ItemTemplate\TempName = "nvg3" Lor Inventory(i)\ItemTemplate\TempName = "nvg2")) Lor ((wi\SCRAMBLE = 1 And Inventory(i)\ItemTemplate\TempName = "scramble") Lor (wi\SCRAMBLE = 2 And Inventory(i)\ItemTemplate\TempName = "scramble2"))
							...
							ElseIf me\Sanity < -500.0
								If Rand(7) = 1 Then EntityTexture(t\OverlayID[Orl_NVG], t\OverlayTextureID[Orl_Txt_NVG])
								If Rand(50) = 1
									EntityTexture(t\OverlayID[Orl_NVG], mon_I\MonitorOverlayID[Rand(MONITOR_895_OVERLAY_1, MONITOR_895_OVERLAY_6)])
									For i = 0 To MaxItemAmount - 1
										If Inventory(i) <> Null
			...
			Case e_cont1_914
				...
				If PlayerRoom = e\room
					...
					If e\EventState2 = 1.0 Then ShouldPlay = MUS_CONT_914
					EntityPick(Camera, 1.0)
					For i = 0 To 1
						If PickedEntity() = e\room\Objects[i]
							DrawHandIcon = True
							If HitKeyUse Then GrabbedEntity = e\room\Objects[i]
							Exit
						EndIf
					Next
							...
							If e\EventState > 70.0 * 3.0
								Select(Setting)
									Case ROUGH
										...
										me\BlinkTimer = -10.0
										If ChannelPlaying(e\SoundCHN) Then StopChannel(e\SoundCHN) : e\SoundCHN = 0
										msg\DeathMsg = Format(GetLocalString("death", "914"), Occupation)
										;[End Block]
									Case COARSE
										;[Block]
										me\BlinkTimer = -10.0
			...
			Case e_cont2_008
				...
				If PlayerRoom = e\room
					If EntityY(me\Collider) < (-4496.0) * RoomScale
						...
						ElseIf e\EventState = 1.0
							...
							If Dist < 4.0
								...
								If me\Bloodloss > 0.0 And I_008\Timer = 0.0 Then I_008\Timer = 0.001
							EndIf
					...
					EndIf
					PlayerPosX = EntityX(me\Collider, True) : PlayerPosY = EntityY(me\Collider, True) : PlayerPosZ = EntityZ(me\Collider, True)
					PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[9]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[10]))
					ToMTElevatorFloor = LowerFloor
					e\EventState3 = UpdateMTElevators(e\EventState3, e\room\RoomDoors[3], e\room\RoomDoors[4], e\room\Objects[9], e\room\Objects[10], e)
				EndIf
				;[End Block]
			Case e_cont2_012
				;[Block]
									...
									If EntityVisible(e\room\Objects[0], Camera)
										...
										If Dist < 0.36
											...
											ElseIf e\EventState2 > 70.0 * 85.0 And e\EventState2 - fps\Factor[0] <= 70.0 * 85.0
												msg\DeathMsg = Format(GetLocalString("death", "012"), Occupation)
												Kill(True)
											EndIf
			...
			Case e_cont2_049
				...
				If PlayerRoom = e\room
					If EntityY(me\Collider) > (-2848.0) * RoomScale
						PlayerPosX = EntityX(me\Collider, True) : PlayerPosY = EntityY(me\Collider, True) : PlayerPosZ = EntityZ(me\Collider, True)
						PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[0]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[1]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[2]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[3]))
						ToMTElevatorFloor = LowerFloor
						e\EventState2 = UpdateMTElevators(e\EventState2, e\room\RoomDoors[0], e\room\RoomDoors[1], e\room\Objects[0], e\room\Objects[1], e)
						e\EventState3 = UpdateMTElevators(e\EventState3, e\room\RoomDoors[2], e\room\RoomDoors[3], e\room\Objects[2], e\room\Objects[3], e)
					Else
...
;							EntityType(it\Collider, HIT_ITEM)
						...
						Else
							If e\EventState > (-70.0) * 4.0
								...
								EndIf
							Else
								me\BlurTimer = 800.0 : me\ForceMove = 0.5 : me\Bloodloss = 0.0
								Pvt = CreatePivot()
...
;							EntityType(it\Collider, HIT_ITEM)
					...
					EndIf
					PlayerPosX = EntityX(me\Collider, True) : PlayerPosY = EntityY(me\Collider, True) : PlayerPosZ = EntityZ(me\Collider, True)
					PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[0]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[1]))
					ToMTElevatorFloor = LowerFloor
					e\EventState2 = UpdateMTElevators(e\EventState2, e\room\RoomDoors[0], e\room\RoomDoors[1], e\room\Objects[0], e\room\Objects[1], e)
				EndIf
				;[End Block]
								...
								If IsItemGoodFor1162ARC(itt)
									Select(Inventory(e\EventState2)\ItemTemplate\TempName)
										...
										Case "paper", "oldpaper"
											...
											;[End Block]
										Case "gasmask", "gasmask2", "gasmask3", "gasmask4", "hazmat", "hazmat2", "hazmat3", "hazmat4"
											;[Block]
											If itt\TempName = "gasmask" Lor itt\TempName = "gasmask2" Lor itt\TempName = "gasmask3" Lor itt\TempName = "gasmask4" Lor itt\TempName = "hazmat" Lor itt\TempName = "hazmat2" Lor itt\TempName = "hazmat3" Lor itt\TempName = "hazmat4" And Rand(2) = 1 Then ShouldCreateItem = True
											;[End Block]
										...
										Case "vest", "vest2"
											;[Block]
											If itt\TempName = "vest" Lor itt\TempName = "vest2" And Rand(1) = 1 Then ShouldCreateItem = True
											;[End Block]
										Default
											;[Block]
			...
			Case e_room2_checkpoint
				;[Block]
				If e\EventState = 0
					If e\room\RoomTemplate\RoomID = r_room2_checkpoint_lcz_hcz
						If szl\CurrentZone = HCZ
							e\room\RoomDoors[0]\Open = True
							e\room\RoomDoors[1]\Open = False
						Else
...
;							For i = 0 To 3
;								If e\room\AdjDoor[i] <> Null
;									e\room\AdjDoor[i]\Open = False
;								EndIf
;							Next
							If e\room\RoomDoors[0]\Open
								OpenCloseDoor(e\room\RoomDoors[0], True)
							ElseIf e\room\RoomDoors[1]\Open
								OpenCloseDoor(e\room\RoomDoors[1], True)
							EndIf
				...
				Else
					If PlayerRoom = e\room
						If e\room\RoomDoors[0]\OpenState = 0 And e\room\RoomDoors[1]\OpenState = 0
							If szl\CurrentZone = HCZ
								If e\room\RoomTemplate\RoomID = r_room2_checkpoint_lcz_hcz
									ChangeZone(LCZ, True)
								Else
									ChangeZone(EZ, True)
								EndIf
							ElseIf szl\CurrentZone = LCZ Lor szl\CurrentZone = EZ
								ChangeZone(HCZ, True)
							EndIf
						EndIf
					EndIf
				...
				If PlayerRoom = e\room
					If InteractObject(e\room\Objects[0], 2.25)
						...
						If e\EventState2 = 2.0
							...
							I_294\Using = Temp
							If I_294\Using Then HitKeyUse = False
			...
			Case e_gate_b_entrance
				...
				If PlayerRoom = e\room
					...
					Else
						...
						PlayerPosX = EntityX(me\Collider, True) : PlayerPosY = EntityY(me\Collider, True) : PlayerPosZ = EntityZ(me\Collider, True)
						PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[0]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[1]))
						ToMTElevatorFloor = UpperFloor
						e\EventState = UpdateMTElevators(e\EventState, e\room\RoomDoors[0], gateb\RoomDoors[1], e\room\Objects[0], e\room\Objects[1], e)
			...
			Case e_gate_a_entrance
				...
				If PlayerRoom = e\room
					...
					Else
						...
						PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[0]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[1]))
						ToMTElevatorFloor = UpperFloor
						e\EventState = UpdateMTElevators(e\EventState, e\room\RoomDoors[0], gatea\RoomDoors[1], e\room\Objects[0], e\room\Objects[1], e)
						If (Not n_I\Curr106\Contained)
			...
			Case e_room2_ez_035
				...
				If Is035Released
					If e\room\Dist < 8.0
						...
						If EntityDistanceSquared(e\room\NPC[0]\Collider, me\Collider) < 6.25
							If wi\HazmatSuit = 0
								DamagePlayer(fps\Factor[0] / 500.0)
							Else
								DamagePlayer(fps\Factor[0] / 1000.0)
							EndIf
							If e\room\NPC[1] = Null
				...
				If e\room\NPC[1] <> Null
					If e\room\NPC[1]\HP = 0 Then RemoveEvent(e)
				EndIf
				;[End Block]
			Case e_room2_tesla
				;[Block]
					...
					Select(e\EventState)
						Case 0.0 ; ~ Idle state
							...
							If Abs(EntityX(me\Collider, True) - EntityX(e\room\OBJ, True)) < 1.0 And Abs(EntityZ(me\Collider, True) - EntityZ(e\room\OBJ, True)) < 1.0 And Abs(EntityY(me\Collider, True) - EntityY(e\room\OBJ, True)) < 1.3
								If me\Health > 0 And (Not chs\NoTarget)
									If e\room\NPC[0] = Null Lor e\room\NPC[0]\HP = 0
										If ChannelPlaying(e\SoundCHN) Then StopChannel(e\SoundCHN) : e\SoundCHN = 0
							...
							EndIf
							For n.NPCs = Each NPCs
								If n\NPCType <> NPCType966 And n\NPCType <> NPCType513_1 And n\HP > 0
						...
						Case 2.0 ; ~ Zap state
							...
							If Abs(EntityX(me\Collider, True) - EntityX(e\room\OBJ, True)) < 0.75 And Abs(EntityZ(me\Collider, True) - EntityZ(e\room\OBJ, True)) < 0.75 And Abs(EntityY(me\Collider, True) - EntityY(e\room\OBJ, True)) < 1.3
								If me\Health > 0
									me\LightFlash = 0.4
									me\CameraShake = 1.0
									msg\DeathMsg = Format(GetLocalString("death", "tesla"), Occupation)
									Kill()
								EndIf
							EndIf
							For n.NPCs = Each NPCs
									...
									If Abs(EntityX(n\Collider, True) - EntityX(e\room\OBJ, True)) < 0.5 And Abs(EntityZ(n\Collider, True) - EntityZ(e\room\OBJ, True)) < 0.5 And Abs(EntityY(n\Collider, True) - EntityY(e\room\OBJ, True)) < 1.3
										...
										Select(n\NPCType)
											Case NPCType106
												...
												EndIf
												;[End Block]
											Case NPCType049, NPCType096, NPCType173, NPCType066
												;[Block]
												; ~ Skip
												;[End Block]
											Default
							...
							Next
							If Rand(5) < 5
								PositionTexture(t\OverlayTextureID[Orl_Txt_Tesla], 0.0, Rnd(0.0, 1.0))
								If EntityHidden(e\room\Objects[0]) Then ShowEntity(e\room\Objects[0])
			...
			Case e_room2_mt
				...
				If PlayerRoom = e\room
							...
							For iX = 0 To MTGridSize - 1
								If e\room\mt\Grid[iX + (iY * MTGridSize)] > 0
									...
									ElseIf e\room\mt\Grid[iX + (iY * MTGridSize)] = MT_FIRST_ELEVATOR Lor e\room\mt\Grid[iX + (iY * MTGridSize)] = MT_SECOND_ELEVATOR
										...
										dr.Doors = CreateDoor(Null, e\room\x + (iX * 2.0) + (CosValue * 256.0 * RoomScale), e\room\y + MTGridY, e\room\z + (iY * 2.0) + (SinValue * 256.0 * RoomScale), EntityYaw(TempInt, True) - 90.0, False, MT_ELEVATOR_DOOR)
					...
					PlayerPosX = EntityX(me\Collider, True) : PlayerPosY = EntityY(me\Collider, True) : PlayerPosZ = EntityZ(me\Collider, True)
					PlayerInsideMTElevator = (IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[2]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[3]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[4]) Lor IsInsideMTElevator(PlayerPosX, PlayerPosY, PlayerPosZ, e\room\Objects[5]))
					ToMTElevatorFloor = UpperFloor
					e\EventState2 = UpdateMTElevators(e\EventState2, e\room\RoomDoors[0], e\room\RoomDoors[1], e\room\Objects[2], e\room\Objects[3], e, False)
					e\EventState3 = UpdateMTElevators(e\EventState3, e\room\RoomDoors[2], e\room\RoomDoors[3], e\room\Objects[4], e\room\Objects[5], e, False)
				Else
					If e\room\mt <> Null
					...
					Else
						...
						Else
							If EntityDistanceSquared(e\room\Objects[0], me\Collider) < 4.0
								...
								EndIf
								DamagePlayer(fps\Factor[0] * 0.0005)
							EndIf
						EndIf
					EndIf
				EndIf
...
;								ItemAmount = ItemAmount + 1
				...
				If PlayerRoom = e\room
					...
					If EntityY(me\Collider) < (-4600.0) * RoomScale
						...
						If wi\GasMask = 0 And wi\HazmatSuit = 0
							...
							If me\BlurTimer >= 1500.0 And me\FallTimer = 0.0 And me\Health > 0
								msg\DeathMsg = Format(GetLocalString("death", "939.gas"), Occupation)
								Kill()
							EndIf
						EndIf
						
						ShouldPlay = MUS_STORAGE
						...
						Else
							...
							If EntityY(me\Collider) < -6400.0 * RoomScale
								If (Not chs\GodMode) And me\health > 0 And me\FallTimer >= 0.0
									msg\DeathMsg = Format(GetLocalString("death", "939.shaft"), Occupation)
							...
							If EntityX(me\Collider) < Max(EntityX(e\room\Objects[2], True), EntityX(e\room\Objects[3], True))
								If EntityZ(me\Collider) > Min(EntityZ(e\room\Objects[2], True), EntityZ(e\room\Objects[3], True))
									If EntityZ(me\Collider) < Max(EntityZ(e\room\Objects[2], True), EntityZ(e\room\Objects[3], True))
										...
										If me\Health = 0 And me\Bloodloss >= 100.0 Then msg\DeathMsg = Format(GetLocalString("death", "035"), Occupation)
									EndIf
								EndIf
							EndIf
						EndIf
			...
			Case e_room2_5_hcz_106
				...
				If (Not n_I\Curr106\Contained)
					...
					ElseIf e\EventState = 1.0
						...
						EndIf
					Else
						If e\EventState2 = 1.0 Then ShouldPlay = MUS_CHASE_106
						e\EventState = e\EventState + fps\Factor[0]
				...
				If e\EventState = 1.0
					...
					ElseIf e\EventState2 = 6.0
						...
						Else
							...
							If PlayerRoom = e\room Then ShouldPlay = MUS_CHASE_049
							If e\room\NPC[0]\PathStatus <> PATH_STATUS_FOUND
			...
			Case e_room2_medibay
				...
				If PlayerRoom = e\room
					...
					Else
						If InteractObject(e\room\Objects[1], 0.49) And me\Health < 100
							CreateMsg(GetLocalString("msg", "breeze"))
							HealPlayer(Rand(10, 15))
							me\Bloodloss = 0.0
			...
			Case e_room2_shaft
				...
				If PlayerRoom = e\room
					...
					If d_I\ClosestButton = e\room\Objects[1] And HitKeyUse
		...
		If e\EventID = e_dimension_106
			...
			If PlayerRoom = e\room
				...
				If e\EventState2 <> PD_FakeTunnelRoom
					ShouldPlay = MUS_DIMESION_106
					
					DamagePlayer(fps\Factor[0] * (0.0005 * (1.0 + (wi\NightVision > 0 Lor wi\SCRAMBLE > 0))))
				...
				Select(e\EventState2)
					...
					Case PD_FourWayRoom
							...
							For i = 9 To 10
								...
								If Dist < 36.0
									If Dist < PowTwo(100.0 * RoomScale)
										...
										If (Not chs\GodMode) And me\Health > 0
						...
						If EntityY(me\Collider) < (-1600.0) * RoomScale
							If EntityDistanceSquared(me\Collider, e\room\Objects[8]) > PowTwo(4750.0 * RoomScale) And me\Health > 0
								Teleport = True
					...
					Case PD_ThroneRoom
						...
						DamagePlayer(fps\Factor[0] / 400.0)
					...
					Case PD_TrenchesRoom
						;[Block]
						ShouldPlay = MUS_DIMESION_106_TRENCHES
						...
						ElseIf Dist < 64.0 And I_714\Using <> 2 And wi\GasMask <> 4 And wi\HazmatSuit <> 4
							...
							If I_714\Using = 1
								DamagePlayer((8.0 - SqrValue) * (fps\Factor[0] * 0.002))
							Else
								DamagePlayer((8.0 - SqrValue) * (fps\Factor[0] * 0.004))
							EndIf
						...
						ElseIf Dist < 64.0
							EntityTexture(e\room\Objects[19], e\room\Textures[0])
							DamagePlayer((8.0 - Sqr(Dist)) * (fps\Factor[0] * 0.001))
						EndIf
			...
			Case e_gate_b
				...
				If PlayerRoom = e\room
					If e\EventState = 0.0
						...
						e\room\Objects[0] = LoadMesh_Strict("GFX\Map\Rooms\exit1terrain.b3d", e\room\OBJ)
						...
						Sky = CreateSky("GFX\Map\Textures\Sky\sky")
			...
			Case e_gate_a
				...
				If PlayerRoom = e\room
					If e\EventState = 0.0
						...
						e\room\Objects[0] = LoadRMesh("GFX\Map\Rooms\gate_a_tunnel.rmesh", Null)
												...
												ElseIf e\EventState2 > 0.0
													...
													If e\EventState2 >= 70.0 * 7.5
														...
														ElseIf e\EventState2 < 70.0 * 14.3
															...
															If (Not (me\health = 0 Lor chs\GodMode))
...
Function Update035Label%(OBJ%)
	...
	achv\AchvIMG[Achv035] = LoadImage_Strict("GFX\Achievements\Achv" + CurrTex + ".png")
