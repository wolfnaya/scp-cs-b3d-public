...
Function UpdateWorld2%()
	...
	If (wi\NightVision > 0 And wi\NightVision <> 3) Lor wi\SCRAMBLE > 0
		For i = 0 To MaxItemAmount - 1
			If Inventory(i) <> Null
				If (wi\NightVision = 1 And Inventory(i)\ItemTemplate\TempName = "nvg") Lor (wi\NightVision = 2 And Inventory(i)\ItemTemplate\TempName = "nvg3") Lor (wi\SCRAMBLE = 1 And Inventory(i)\ItemTemplate\TempName = "scramble") Lor (wi\SCRAMBLE = 2 And Inventory(i)\ItemTemplate\TempName = "scramble2")
...
Function RenderWorld2%(Tween#)
	...
	If HasBattery > 0
		...
		Next
		DrawImage(t\ImageID[Img_NVG_Battery], 40 * MenuScale, mo\Viewport_Center_Y + (30 * MenuScale))
	EndIf
...
Function PlayStartupVideos%()
	...
	Local MovieFile$, i%
	Local StartupPath$ = "GFX\Menu\Videos\"
	
	For i = 0 To 3
		Select(i)
			Case 0
				;[Block]
				MovieFile = StartupPath + "startup_NWI"
				;[End Block]
			Case 1
				;[Block]
		...
		Repeat
			...
			If GetAnyKey() Lor (Not IsStreamPlaying_Strict(SplashScreenAudio));GetKey() <> 0 Lor MouseHit(1) Lor (Not IsStreamPlaying_Strict(SplashScreenAudio))
		...
		If b <> 0
			For j = 0 To 7
				...
				If t1 <> 0
					...
					If Left(Lower(Name),LetterNumber) = TexAndMatName$
